...
export async function resolveStaffAccess(
  ...
  const { data, error } = await supabase
    ...
    .select(
      "barber_id, can_see_earnings, can_confirm, can_cancel, can_contact_client, can_create_appointment, can_block_time, barbers(id, name, commission_percent, is_active, deleted_at)",
    )
